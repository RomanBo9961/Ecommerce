...
public class SpringBootSecurity {
	...
	SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http.authorizeRequests(requests -> requests.requestMatchers("/administrador/**").hasRole("ADMIN")
                ...
                /*
                 * .csrf(csrf ->
                 * csrf.disable()).formLogin().loginPage("/usuario/login").permitAll()
                 * .defaultSuccessUrl("/usuario/acceder").and().logout().permitAll();
                 */
                .csrf(csrf -> csrf.disable()).formLogin(login -> login.loginPage("/usuario/login").permitAll()
                .defaultSuccessUrl("/usuario/acceder")).logout(logout -> logout.permitAll());
		return http.build();
	}
